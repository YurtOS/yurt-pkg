...
#[test]
fn assert_not_published_rejects_existing_version_without_artifact() {
    let dir = tempfile::tempdir().unwrap();
    let repo = dir.path().join("repo");
    let inputs = dir.path().join("inputs");
    fs::create_dir_all(&inputs).unwrap();
...
"#,
    ...
    let mut check = Command::cargo_bin("yurt-repo-ci").unwrap();
    check.args([
    ...
    check.assert().failure().stderr(predicates::str::contains(
        "yurt-greet 0.1.0-yurt_0 is already published",
    ));
}
